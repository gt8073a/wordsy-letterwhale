...
module.exports = function(args) {
	...
	self.onYourMarks = function(fn) {
		...
		self._timeouts.STARTING = setTimeout( function() { self.services.SEND_MSG_FN( 'Game beginning', 'alert', marksFn ) }, 1000 );
	...
	self.runGame = function( config ) {
		...
		try {
			...
			self.services.SEND_MSG_FN( e, 'highlight|alert' )
			return;
		}
		...
		self.services.SEND_MSG_FN( reps, 'highlight' );
	...
	self.gameOver = function() {
		...
		self.services.SEND_MSG_FN( 'GAME OVER', 'alert', self.scoreBoard );
	...
	self.setPeriods = function( ) {
		...
		self._timeouts.PERIOD = setTimeout(
							function() {
								self.services.SEND_MSG_FN( 'Second half! Words are worth more.', bonusFn );
	...
	self.gameOverCountDown = function() {
		...
		var twoFn = function(bot,resp ) {
			self._timeouts.GAME_OVER = setTimeout( function() {self.services.UPDATE_MSG_FN( resp, 'Game over in 2', oneFn )}, 1500 )
                ...
                self._timeouts.GAME_OVER = setTimeout( function() { self.services.SEND_MSG_FN( 'Game over in 3..', 'alert', twoFn ) }, 4000 );
	...
	self.scoreBoard = function() {
		...
		self.services.SEND_MSG_FN(thisMsg);
