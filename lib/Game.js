...
module.exports = function(args) {
	...
	self.SEND_ALERT_FN   = args.SEND_ALERT_FN   || function() { return true };
	...
	self.setPointMultiplierTimeouts = function() {
		self._timeouts.START_SECOND_PERIOD = setTimeout(
							function() { self.SEND_MSG_FN( 'Second half! Words are worth more.' ); self.POINTS_MULTIPLIER = 1.5 },
							( self.TIME_OF_GAME * 1000 / 2 )
						)

		self._timeouts.START_BONUS_PERIOD  = setTimeout(
							function() { self.SEND_MSG_FN( 'Bonus! Words are worth even more.' ); self.POINTS_MULTIPLIER = 2 },
	...
	self.onYourMarks = function(fn) {
		...
		self._timeouts.START_IN_THREE = setTimeout( function() { self.SEND_MSG_FN( 'On your marks', marksFn ) }, 1000 );
	}
	...
	self.runGame = function() {
		...
		self.SEND_ALERT_FN( self.letterBoard.getText() )
	...
	self.gameOver = function() {
		...
		self.SEND_ALERT_FN( 'GAME OVER', self.scoreBoard );
	...
	self.tickTockTickTock = function() {
		...
		var oneFn = function( bot,resp ) {
			bot.api.chat.update({ ts: resp.ts, channel: resp.channel, text: '```1!!```' }, function( err, resp ) {
				self._timeouts.GAME_OVER = setTimeout( self.gameOver, 1500 );
			})
		};
