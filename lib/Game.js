...
module.exports = function(args) {
	...
	self.runGame = function( config ) {
		...
		var reps = [
			...
			{ text: ' - ', level: 'plain' },
			{ text: self.getTimeLeft(0), level: 'italics' },
			{ text: ' | ', level: 'plain' },
			{ text: self.playerLog.words.MIN_WORD_LENGTH + '+ letter words', level: 'italics' }
		];
	...
	/* needs a timer */
	self.getTimeLeft = function( perc ) {
		...
		if ( self.STATE == 'GAME OVER' ) {
			...
			left = "Game's just starting.. " + self.time + ' seconds';
		} else {	
			var now         = new Date(),
			    nowTime     = now.getTime(),
